  ...
  for (let row = 0; row <= rings; row++) {
    ...
    for (let side = 0; side <= sides; side++) {
      ...
      if (plumage) {
        ...
        // Small deterministic changes follow the surface; there is no texture download.
        color.lerp(mottling, .09 * (1 + Math.sin(row * 8.7 + side * 5.3)));
...
export function createTimelineCompanion(materials: TimelineMaterials) {
  ...
  const feather = new THREE.MeshStandardMaterial({ color: "#503a29", roughness: .92, metalness: 0 });
  const featherDark = new THREE.MeshStandardMaterial({ color: "#30261e", roughness: .94, metalness: 0 });
  const featherLight = new THREE.MeshStandardMaterial({ color: "#79604b", roughness: .92, metalness: 0 });
  const down = new THREE.MeshStandardMaterial({ color: "#ffffff", vertexColors: true, roughness: .96, metalness: 0 });
  const cream = new THREE.MeshStandardMaterial({ color: "#cbbda7", roughness: .94, metalness: 0 });
  const horn = new THREE.MeshStandardMaterial({ color: "#28221c", roughness: .61, metalness: 0 });
  const legsMaterial = new THREE.MeshStandardMaterial({ color: "#90725d", roughness: .82, metalness: 0 });
  const eyeMaterial = new THREE.MeshStandardMaterial({ color: "#0b0907", roughness: .17, metalness: 0 });
  ...
  for (const side of [-1, 1]) {
    ...
    const eye = mesh(neck, new THREE.SphereGeometry(.019, 16, 12), eyeMaterial);
    eye.position.set(.088, .084, side * .092);
    eye.scale.z = .65;
    eyes.push(eye);
    const catchlight = mesh(eye, new THREE.SphereGeometry(.003, 8, 6), materials.paper);
    catchlight.position.set(.004, .007, side * .015);
  }
  ...
  return {
    ...
    animate(time: number, flight: number, phase: number, work: number, interaction: TimelineCompanionInteraction = {}) {
      ...
      const greeting = THREE.MathUtils.clamp(interaction.greeting ?? 0, 0, 1) * (1 - flying * .65);
      ...
      const working = THREE.MathUtils.clamp(work, 0, 1) * (1 - greeting * .9);
      ...
      if (previousFlight > .2 && flying <= .2) landedAt = time;
      ...
      const lookPhase = ((time % 7.3) + 7.3) % 7.3;
      const glance = lookPhase > 1.2 && lookPhase < 2.6
        ? Math.sin((lookPhase - 1.2) / 1.4 * Math.PI) ** 2
        : lookPhase > 5.1 && lookPhase < 5.95 ? -.65 * Math.sin((lookPhase - 5.1) / .85 * Math.PI) ** 2 : 0;
      torso.position.y = .004 * breathe * (1 - effort) - .025 * landing;
      torso.rotation.z = -.065 * flying - .045 * effort + .035 * greeting;
      ...
      const attention = 1 - action.grip * working;
      neck.rotation.x = .20 * greeting + .08 * action.inspect * working;
      neck.rotation.z = working * (-.08 * action.inspect - .065 * effort + .065 * action.release)
        + (.04 * glance + .23 * gazeY) * attention + .075 * greeting;
      neck.rotation.y = (.19 * glance + .5 * gazeX) * attention + .13 * action.inspect * working;
      neck.scale.y = 1 + .045 * action.reach * working;
      lowerBeak.rotation.z = -.16 * action.gape * working;
      const wingPhase = ((time * 5.4) % 1 + 1) % 1;
      const settlePhase = ((time % 6.7) + 6.7) % 6.7;
      const featherSettle = settlePhase > 2.1 && settlePhase < 3.1
        ? Math.sin((settlePhase - 2.1) * Math.PI) ** 2 * (1 - flying) * (1 - working * action.reach)
        : 0;
      ...
      wings.forEach(({ shoulder, elbow, wrist, primaries, side }) => {
        const wingGreeting = greeting * (side > 0 ? 1 : .72);
        shoulder.rotation.x = side * (flying * (.65 + stroke * 1.12) + featherSettle * (side > 0 ? .3 : .12) + wingGreeting * .48);
        shoulder.rotation.y = side * (.05 + flying * 1.13 + wingGreeting * .58);
        shoulder.rotation.z = -.11 * flying + .015 * breathe;
        elbow.rotation.z = flying * (-.2 + recovery * .48) - wingGreeting * .12;
        elbow.rotation.x = side * flying * recovery * .55;
        wrist.rotation.y = side * (flying * (.12 - recovery * .85) + wingGreeting * .15);
        wrist.rotation.z = flying * recovery * .4 + featherSettle * .15 - wingGreeting * .1;
        primaries.forEach((primary, i) => {
          primary.rotation.z = -.12 + i * .065 + flying * (1 - recovery * .7) * i * .034 + wingGreeting * i * .03;
        });
      });
      tail.rotation.z = .025 * glance + .11 * flying + .045 * effort + .06 * landing + .06 * greeting;
      ...
      legs.forEach(({ hip, ankle, toes, side }) => {
        const stepping = side > 0 ? Math.max(action.inspect * working, greeting) * (1 - flying) : 0;
      ...
      const blinkWave = Math.sin(time * .81);
      const blink = 1 - .94 * THREE.MathUtils.smoothstep(blinkWave, .991, 1);
